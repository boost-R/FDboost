...
\item
\texttt{year}:
A vector of length 140 containing the integers from 1950 to 2019 two times (\texttt{c(1950:2019, 1950:2019)}), corresponding to the years for the rows of \texttt{birth\_densities} and \texttt{birth\_densities\_clr}.
\item
\texttt{month}:
...
This is rather time-consuming and preferably should be executed parallelized on multiple cores.
<<get-mstop, echo = TRUE, message=FALSE>>=
set.seed(1708)
folds <- applyFolds(model, folds = cv(rep(1, model$ydim[1]), type = "bootstrap", B = 10))
model <- model[mstop(folds)] # 999
# mstop <- 999
@
...
\item[] Maier, E.-M., St\"ocker, A., Fitzenberger, B., Greven, S. (2021). Boosting flexible regression models for compositional data and probability densities in Bayes Hilbert spaces. \textit{Journal} vol(number), pages.
\end{itemize}
